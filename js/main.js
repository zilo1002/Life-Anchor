...
function closeMessage(e) {
    // Don't close if clicking on the message card or its children
...
function setupEventListeners() {
    // Main button click - ensure it works reliably
    anchorButton.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();

        // Always call activateButton regardless of current state
        activateButton(e);
    });

    // Also support touch for mobile
    anchorButton.addEventListener('touchend', function(e) {
        e.preventDefault();
        e.stopPropagation();
        activateButton(e);
    });
    ...
    // Close message when clicking outside - but not on the button
    document.addEventListener('click', (e) => {
        if (!messageCard.classList.contains('visible')) return;

        // Check if click is on message card or its children (including nav buttons)
        const isClickOnCard = messageCard.contains(e.target) && e.target !== anchorButton;
        // Check if click is on anchor button
        const isClickOnButton = e.target === anchorButton || anchorButton.contains(e.target);

        // Don't close if clicking on card or button
