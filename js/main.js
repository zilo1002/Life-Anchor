...
function getDailyPool() {
    ...
    if (saved && savedDate === dateString) {
        try {
            const parsed = JSON.parse(saved);
            // 验证数据格式是否正确（包含必要字段）
            if (Array.isArray(parsed) && parsed.length > 0 && parsed[0].zh && parsed[0].source) {
                state.dailyPool = parsed;
                return state.dailyPool;
            }
        } catch (e) {
            console.warn('缓存数据无效，重新生成');
        }
    }
...
function getCurrentContent() {
    ...
    switch(state.language) {
        case 'zh':
            return dailyContent.map(item => ({ story: item.zh || item.en || '', source: item.source || '未知来源' }));
        case 'en':
            return dailyContent.map(item => ({ story: item.en || item.zh || '', source: item.source || 'Unknown' }));
        case 'both':
            return dailyContent;
        default:
            return dailyContent.map(item => ({ story: item.zh || item.en || '', source: item.source || '未知来源' }));
...
function showCurrentContent(direction = null) {
    ...
    if (state.language === 'both') {
        mainMessage.innerHTML = `<span class="zh-text">${content.zh || content.story || ''}</span><span class="en-text">${content.en || ''}</span>`;
    } else {
        mainMessage.textContent = content.story || content.zh || content.en || '内容加载中...';
    }
    storySource.textContent = content.source || '未知来源';
}
